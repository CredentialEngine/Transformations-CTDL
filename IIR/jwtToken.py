...
- Challenge JSON must include: didkey, challenge, aud, iat, exp
  - didkey: verification method id / DID URL with fragment; used as JWT header "kid"
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Sign a CE-generated challenge JSON file as a Proof JWT.")
    ...
    ap.add_argument(
        "--challengeFile",
        required=True,
        help="Path to CE-generated challenge JSON file",
    )
    ...
    # Accept either 'didkey' or 'didKey' (common casing variants)
    didkey = payload_obj.get("didkey")
    if didkey is None:
        didkey = payload_obj.get("didKey")

    if not isinstance(didkey, str) or not didkey.strip():
        raise SystemExit('Challenge JSON must include a non-empty string field "didkey" (or "didKey").')

    # Required fields
    required_fields = ["challenge","aud", "iat", "exp"]
    ...
    # IMPORTANT: Payload is NOT modified (including didkey)
